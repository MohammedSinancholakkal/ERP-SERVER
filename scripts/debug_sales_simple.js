...
async function run() {
  try {
    ...
    // 1. Get Latest Sale VNo
    const saleRes = await sql.query`SELECT TOP 1 VNo FROM Sales ORDER BY Id DESC`;
    const vNo = saleRes.recordset[0]?.VNo;
    console.log("Sale VNo:", vNo);
