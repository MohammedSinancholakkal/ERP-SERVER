...
async function verifyJournalTransaction() {
  try {
    const pool = await sql.connect(config);
    console.log('Connected to MSSQL');

    // 1. Get Latest Journal Voucher
    ...
    if (voucherRes.recordset.length === 0) {
        console.log("No Journal Vouchers found. Please add one via the UI or API script first.");
        process.exit(0);
    }

    const voucher = voucherRes.recordset[0];
    console.log("\n--- LATEST JOURNAL VOUCHER ---");
    ...
    if (transRes.recordset.length > 0) {
        ...
        if (isMatch) {
            ...
            if(trans.VType !== 'Journal Voucher') console.log(`Expected VType 'Journal Voucher', got '${trans.VType}'`);
